...
ポインタの概念はあるものの、C言語のそれと異なり使い方を間違えるとメモリ破壊やセグメンテーションフォールトの原因となるようなものではなく、言語仕様もコンパクトで身につけやすく、処理によっては普通に書いてもC++に比肩する速度を出せるため、プログラミングを本業としない科学者の利用には非常に適していると思っていたのですが、なぜこのような状況になっているのでしょうか。
...
@<href>{http://www.gonum.org/,Gonum（http://www.gonum.org/）} というものがあります。内部的にはBLAS（Basic Linear Algebra Subprograms）とLAPACK（Linear Algebra PACKage）という、わりと昔から存在するベクトルや行列、線型代数でよく用いる演算の共通ルーチン集のようなライブラリの、インターフェースを維持したGo版実装となっています。
元の実装がC言語やFORTRANであるため、歴史を感じる作りになっています。

=== 他言語との比較
2018年現在において、数値計算に関してPythonに言及することは避けられないでしょう。

機械学習でよく用いられるPythonですが、さまざまな計算においてNumPyやSciPyが用いられています。非常に高度なことばかり行っていて常人には理解できない計算ばかりという先入観を持ってしまうのですが、たとえばニューラルネットワークの計算も大雑把にいってしまえば座標変換と活性化関数の作用を大量に繰り返しているだけなので、高等数学といっても大学1,2年生なら理解できるレベルではあるはずです。
...
ただ、プログラミングを専門としない科学者がプログラミングを学ぶための教材という観点で考えると、PythonよりもGo言語は合理的である可能性はあります。

C言語やPythonよりも型付けが強く、コンパイラやlint/vetが優秀であり、かつ多くの場合局所的なコードは誰が書いても同じようなものになるため、本来のロジック実装に集中できるというところが非常に優位であると考えます。
筆者の経験ですが、プログラミングの必須科目も存在していなかったのに、物理の専門課程の演習に入った瞬間にC言語でフーリエ変換を Numerical Recipe in C を読みながら実装することになったのですが、ありとあらゆる壁にぶつかり泣く泣く同期のコードをだいぶ使わせてもらった記憶があります……
あの頃にGo言語があったらなと、職業プログラマとして何年か働きプログラミングについて一定以上の理解を得た今、強く思う次第です。
 ...
 ** パフォーマンスについては追加調査が必要
 * 科学者がC言語を使っているのは歴史的経緯と某社の最適化が強いコンパイラがある環境であることなどが想定されるので、C言語からGo言語に乗り換えるメリットは少ないように考えられる。
 ** Python/NumPy であればメリットが大きい可能性がある。
 * ただし、プログラミングの専門家でないが数値計算のためにプログラミングを学ぶ人の教材としてGo言語からはじめるのは、筆者の経験を振り返るに有力な選択肢になり得る。
 ...
 * @<href>{https://www.amazon.co.jp/dp/4874085601,NUMERICAL RECIPES in C[日本語版]}
