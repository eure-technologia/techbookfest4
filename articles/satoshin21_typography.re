...
しかし、ただ単純に画像を表示する場合に比べ、（画像のメモリ管理は別として）@<b>{文字をアプリケーションで扱うときは様々な問題が発生します。}フォントによってレイアウトが崩れたり、デザイナーの要求に素直に（そしてカッコよく）答えられなかったりなど、なかなか最初の頃は一筋縄ではいきません。

この章ではタイポグラフィについて、SwiftとiOS開発の観点からざっくりと理解する為の手助けを目的としています。iOSエンジニアにとってはもちろん、タイポグラフィについての基礎の部分を理解したい人のお役に立てればと思います。

== 書体の歴史と活版
タイポグラフィはさまざまな定義がありますが、一般的な概念としては「文字をページ上に美しく配置する為の技法や工程」のことを指します。iOS開発もUIFontを用い、UITextViewやUILabelなどのView上に配置する為の技法をタイポグラフィとして定義できるかと思います。
...
DTP（DeskTop Publishing）の発達により、人間は膨大な数の文字の組み合わせである活版をフォントという形でデータとして扱うことでパソコン上でとても簡単に文字を印刷物上に表現することができました。Web開発やネイティブアプリケーションにおいても同様で、我々はフォントを1つのセットとして取り扱うことで、膨大な数の文字の形を使い分けてアプリケーションのデザインを行うことができるようになりました。
...
HiraKakuProN-W3などは、line heightを超えてしまうLine Gapを含めないと実際のフォントの高さとなりません。その為、iOSにおいてフォントの持つ正確な1行分の高さを取得する場合は、Line Gapを含めて計算する必要があります。
...
@<b>{日本語は四角形の中に収まる形でグリフを表し、欧文フォントのようにbaseline上に一定のラインを基準としたグリフ表現はしていません。}その為、UIFontで取得できるascenderなどの値は日本語には適用されませんが、欧文フォントに合わせる形でグリフの位置が定義されている為、leadingを考慮した高さを取得すれば、基本的にはみ出てしまうことはありません。
...
UITextView上に表示するテキストの座標と形状情報を持っています。TextView上のスクロール可能な領域に関するレイアウト情報を持っているようなイメージで、座標の主な情報は以下のような関係性となっています。
...
またTextContainerはNSAttributedStringでカバーできない、テキスト全体の描画方法についても指定することができます。
...
柔軟なタイポグラフィをiOS上で実現するために、UITextViewは様々なAPIを提供しているため、各クラスの特性を掴み快適なタイポグラフィを実現しましょう。
