...
ブロックチェーンという言葉を最近聞くようになりましたね。
ブロックチェーンとは、「みんなで分散して管理する技術」のことです。

この章では、ブロックチェーンについて、そしてブロックチェーンを実装したHyperledger Fabricを
...
ブロックチェーンは、取引情報を書き込んだブロックをつなげた情報が書き込まれる台帳のことです。

具体的な手続きとしては
とあるノード（サーバーインスタンスの台帳）に取引情報が送信されます。
...
2.ネットワーク上の他のノードにブロックを共有し、台帳に書き込まれるようになっていることから、どこかのサーバーの台帳が失われても大丈夫なのです。
...
Try it out!で実際にその場でAPIを実行することもできます。便利ですね。
