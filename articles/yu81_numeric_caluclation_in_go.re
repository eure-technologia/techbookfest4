...
実数 a, b を用いて、複素数 @<b>{z} は以下の様に書けます。
...
また、実部と虚部をそれぞれ2次元の平面の横軸と縦軸にプロットした複素平面を定義し、その原点からの距離と原点から引いた線分とのなす角度を用いて表すこともあります。以下の様に書けます。
...
Go言語では最新バージョンの1.10現在、複素数は組み込み型として利用可能であり、以下の様に利用可能です。
