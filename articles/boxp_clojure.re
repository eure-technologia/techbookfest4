...
しかし、Clojureはその他数多くの言語に一線を成すユニークな特徴を多く持ち、どのプラットフォームでも一貫して以下の恩恵が得られます。

=== 高機能なREPLによる高速なアプリケーション開発

REPL(Read Eval Print Loop)は対話型開発環境とも呼ばれるもので、書いたコードを即評価して確認する事ができるものです。
これそのものは様々な言語で利用されていますが、Clojureやその仲間であるLisp系の言語達は一風変わったREPLを持っており、
その力を最大限に活かした開発を行う事が出来ます。

Clojureでのアプリケーション開発サイクル標準的な開発サイクルは以下のようなものです。
...
エディタやIDEから直接コードを評価し確認する事で、高速な開発サイクルを実現できます:)

他にも様々な特徴がありますが、書きたい事を全部書くとこの章が終わってしまうのでここまでにします。
...
Clojureは、コードのビルド・ライブラリの依存管理・REPLの立ち上げ等のコーディングに必要な周辺環境は基本的に「Leiningen」と言うツールを用いて行います。
まずはLeiningenをインストールする所から初めて行きましょう。

Leiningenはhomebrewなどのパッケージマネージャから簡単にインストール出来ます。

//emlist[][]{
homebrew install leiningen
//}

Linux環境の場合、パッケージマネージャを使ってインストールすると古いバージョンがインストールされてしまう可能性があるため、以下のコマンドでインストールする事をおすすめします。
...
結果として@<tt>{3}が返ってくる事が確認出来たでしょうか。
ひとまず、立ち上げたREPLは@<tt>{Ctrl-D}を入力する事で終了できるので一旦終了させておきましょう。
...
これを使う事でWebサーバーやReactを使ったSPA(Single Page Application)のプロジェクトをノータイムで作る事が出来ます。
しかしここでは、簡単の為に最もシンプルなLeiningenに標準搭載されているテンプレートから使って行きましょう。

ボイラープレートを使ってプロジェクトを作るには、以下のように@<tt>{lein}コマンドを呼び出します。

//emlist[][]{
lein new techbookfest
//}

この結果、下のようなプロジェクトが出来上がったと思います。
...
@<tt>{src}フォルダに置かれたソースコードが全て読み込まれた上で起動する事が出来ます。
プロジェクトフォルダ直下で、再度REPLを起動してみましょう。
...
これで簡単なアプリケーションをREPLで開発して行く準備が出来ました！
...
以下のように@<tt>{user=>}の後に続くコードを入力する事で、書いた通りの出力結果が得られる事を確認して行きましょう。
...
注意すべき点として、Clojureにおける変数は必ずImmutable(不変)で再代入が不可能になっていて、RubyやPythonのような一般的な変数と同じ事をする場合はatomを用いる必要があります。
...
;; 変数に束縛する値をatomでくるむことで、一般的な変数と同じことが出来ます
...
Clojureにおけるリスト(List)・ベクター(Vector)・集合(Sets)・ハッシュマップ(HashMap)は@<tt>{シーケンス(Sequences)}と呼ばれる共通のインターフェースを持っていて、以下の共通項を持っています。

	* お互いへの相互変換が可能である
	* 共通して使用できる命令を持つ(map, reduce, forなど)
...
後はClojureの命令が良くまとまったチートシート(https://clojure.org/api/cheatsheet)が用意されているので、このチートシートを傍らに開きながらコーディングする事を勧めます。
...
「duct」は、Clojureを使ったサーバーサイドアプリケーション開発で最も人気のあるフレームワークの一つです。

REPLを使った効率的な開発が可能な他、ボイラープレートがherokuへのデプロイに必要な設定ファイルや静的ウェブページを返すエンドポイントを自動生成してくれる等など大変充実しています。

開発を始めるのなら、ぜひQuickStart(https://github.com/duct-framework/duct#quick-start)を参考に始めると良いでしょう。

=== フロントエンドアプリケーションを作るフレームワーク「re-frame」

「re-frame」はSPA(Single Page Application)を作るClojure向けのフレームワークの中で最も人気のあるもののうちの一つです。

React.jsへの薄いラッパーライブラリのReagentにReduxのようなワークフローを加えたフレームワークですが、著者の経験上他のフレームワークよりも比較的シンプルで扱いやすいです。

もちろんボイラープレートも提供されていて、開発の始め方についてはre-frameのREADME(https://github.com/Day8/re-frame)を読むと良いでしょう。

=== ネイティブアプリケーションを作るフレームワーク「re-natal」

re-natalは、ReactNative上でClojureを向けのフレームワークを使ったフロントエンドアプリケーションを動作させるためのツールキットです。

↑で紹介していたre-frameやその他のフレームワークから一つを選んで開発する事ができるボイラープレートも提供されています。
