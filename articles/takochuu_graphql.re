...
Schemaの定義次第ですが、先程提示したサンプルのように欲しい情報を一度のリクエストで取得することが可能です。これにより、ペイロードのサイズを小さくすることが可能です。@<br>{}
@<br>{}
...
この状態で、さきほど実行したcurlを変更して、下記のようなクエリを投げます。
...
先程のソースコードを改造して、@<code>{graphql.SchemaConfig} に対してMutationフィールドを定義をします。@<br>{}
...
このように、渡した内容に応じてサーバーサイドで計算して値を返す事が可能になっています。@<br>{}
...
REST / GraphQLとどちらが良いではなく適材適所だと思うので、普及したタイミングやサービスへ導入したタイミングでまた話ができればと思います。@<br>{}
@<br>{}
