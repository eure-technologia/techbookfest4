...
ReduxはFluxがベースになっており、基本的な概念は同じですが、次のような違いがあります。
...
ReduxではStoreに渡されたActionに任意の加工を施す仕組みがあり、Middlewareと呼ばれています。たとえば、状態変更の前後でStateをログに出力したり、非同期なActionを実行してから状態変更を行ったり、といった具合に任意の拡張を加えることが可能です。

MiddlewareはActionの前後に割り込むことから、次のようなインターフェースを定義します。
...
大規模なAndroidアプリ開発においては、さまざまなデータソースや状態の種類のハンドリング、非同期処理の扱いが課題となることが多いですが、Reduxを適用することでこれらの課題が解決できるのではないかと思います。今回は紙面の都合で省略した部分もあるので、実際に手を動かしながら検証したい方は次のリポジトリをご覧ください。また、内容について質問したい方は私のTwitterまでメンションをいただければ可能な限り回答いたします。
